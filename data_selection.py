...
def display_logs_selection():
    ...
    if 'datetime' not in df.columns:
        st.sidebar.error("'datetime' 列未找到，请检查数据导入步骤。")
        return
    ...
    # 让用户选择日期范围
    start_date = st.sidebar.date_input("开始日期", value=min_datetime.date())
    end_date = st.sidebar.date_input("结束日期", value=max_datetime.date())
    
    # 确保结束日期不早于开始日期
    if end_date < start_date:
        st.sidebar.error("结束日期不能早于开始日期")
        return
